...
# Sprawdzamy, czy GPU jest dostępne, jeśli tak, ustawiamy je jako urządzenie, w przeciwnym razie używamy CPU
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Informujemy, jakie urządzenie jest używane
...
for i, results in enumerate(results_list):
    ...
    lags = [edge[2] for edge in edges]  # Trzeci element to `max_te_lag`
    past_spans.extend(lags)

# Konwersja na numpy array dla łatwiejszej analizy
past_spans = np.array(past_spans)

# Podstawowe statystyki
print(f"Min pastSpan: {np.min(past_spans)} ms")
print(f"Max pastSpan: {np.max(past_spans)} ms")
print(f"Średni pastSpan: {np.mean(past_spans):.2f} ms")
print(f"Mediana pastSpan: {np.median(past_spans):.2f} ms")
