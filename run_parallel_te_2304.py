...
def process_epoch(epoch):
    ...
    # 1. Pobierz surową macierz
    adj = result.get_adjacency_matrix(weights='max_te_lag', fdr=False)

    # 2. Sprawdź typ i przekonwertuj na zwykłe numpy.ndarray
    print("Typ zwróconej macierzy:", type(adj))
    try:
        adj_array = np.array(adj)
    except Exception:
        # jeśli to scipy.sparse
        adj_array = adj.toarray()

    # 3. Pokaż ją na ekranie
    print("Macierz opóźnień (shape={}):".format(adj_array.shape))
    print(adj_array)
...
def main():
    ...
    with open("logs/combined_results2304.txt", "w") as f:
    ...
    if combined_edges:
        ...
        plt.grid(True)
        plt.savefig("plots/combined_lag_histogram2304.png")
        print("Wyniki zapisano do logs/combined_results2304.txt oraz plots/combined_lag_histogram2304.png")
    else:
